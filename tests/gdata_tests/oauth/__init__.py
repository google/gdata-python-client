...
class OauthUtilsTest(unittest.TestCase):
    def test_build_authenticate_header(self):
        self.assertEqual(oauth.build_authenticate_header(), {'WWW-Authenticate': 'OAuth realm=""'})
        self.assertEqual(oauth.build_authenticate_header('foo'), {'WWW-Authenticate': 'OAuth realm="foo"'})
    ...
    def test_escape(self):
        ...
        self.assertEqual(oauth.escape('Peter Strömberg'), 'Peter%20Str%C3%B6mberg')
...
class OAuthTokenTest(unittest.TestCase):
    ...
    def test_to_string(self):
        self.assertEqual(self.token.to_string(), 'oauth_token_secret=secret&oauth_token=key')
        t = oauth.OAuthToken('+', '%')
        self.assertEqual(t.to_string(), 'oauth_token_secret=%25&oauth_token=%2B')
        
    def test_from_string(self):
        t = oauth.OAuthToken.from_string('oauth_token_secret=secret&oauth_token=key')
    ...
    def test___str__(self):
        self.assertEqual(str(self.token), 'oauth_token_secret=secret&oauth_token=key')
        t = oauth.OAuthToken('+', '%')
...
class OAuthTest(unittest.TestCase):
    def setUp(self):
        self.consumer = oauth.OAuthConsumer('a56b5ff0a637ab283d1d8e32ced37a9c', '9a3248210c84b264b56b98c0b872bc8a')
        self.token = oauth.OAuthToken('5b2cafbf20b11bace53b29e37d8a673d', '3f71254637df2002d8819458ae4f6c51')
...
class OAuthRequestTest(OAuthTest):
    def setUp(self):
        ...
        self.oauth_header_with_realm = {'Authorization': 'OAuth realm="http://example.com", oauth_nonce="1770704051", oauth_timestamp="1278573584", oauth_consumer_key="a56b5ff0a637ab283d1d8e32ced37a9c", oauth_signature_method="HMAC-SHA1", oauth_version="1.0", oauth_token="5b2cafbf20b11bace53b29e37d8a673d", oauth_signature="947ysBZiMn6FGZ11AW06Ioco4mo%3D"'}
        self.oauth_header_without_realm = {'Authorization': 'OAuth oauth_nonce="1770704051", oauth_timestamp="1278573584", oauth_consumer_key="a56b5ff0a637ab283d1d8e32ced37a9c", oauth_signature_method="HMAC-SHA1", oauth_version="1.0", oauth_token="5b2cafbf20b11bace53b29e37d8a673d", oauth_signature="947ysBZiMn6FGZ11AW06Ioco4mo%3D"'}
        
        
        self.additional_param = 'foo'
        self.additional_value = 'bar'
        
        self.oauth_request = oauth.OAuthRequest(self.http_method, self.http_url, self.parameters)
        
    def test_set_parameter(self):
        self.oauth_request.set_parameter(self.additional_param, self.additional_value)
        self.assertEqual(self.oauth_request.get_parameter(self.additional_param), self.additional_value)
        
    def test_get_parameter(self):
        self.assertRaises(oauth.OAuthError, self.oauth_request.get_parameter, self.additional_param)
        self.oauth_request.set_parameter(self.additional_param, self.additional_value)
        self.assertEqual(self.oauth_request.get_parameter(self.additional_param), self.additional_value)
        
    def test__get_timestamp_nonce(self):
        self.assertEqual(self.oauth_request._get_timestamp_nonce(), (self.parameters[OAuthParameters.TIMESTAMP], 
                                                                     self.parameters[OAuthParameters.NONCE]))
    def test_get_nonoauth_parameters(self):
        self.assertTrue(self.oauth_request.get_nonoauth_parameters().has_key(self.non_oauth_param_message))
        self.assertFalse(self.oauth_request.get_nonoauth_parameters().has_key(OAuthParameters.CONSUMER_KEY))
        
    def test_to_header(self):
        realm = 'google'
        header_without_realm = self.oauth_request.to_header().get('Authorization')
        header_with_realm = self.oauth_request.to_header(realm).get('Authorization')
    ...
    def test_to_url(self):
        ...
        self.assertTrue(GET_url.find(self.oauth_request.get_normalized_http_url()) > -1)
    ...
    def test_get_normalized_parameters(self):
        ...
        self.assertFalse(normalized_params.find(OAuthParameters.SIGNATURE + '=') > -1)
        ...
        for k, v in expected_key_values:
            self.assertTrue(expected_key_values.index((k,v)) is key_values.index((oauth.escape(k), oauth.escape(v))))
    ...
    def test_get_normalized_http_method(self):
        ...
        self.assertEqual(self.oauth_request.get_normalized_http_method(), lower_case_http_method.upper())
    ...
    def test_get_normalized_http_url(self):
        ...
        self.assertEqual(self.oauth_request.get_normalized_http_url(), expected_url1)
        
        url2 = 'HTTPS://Example.com:443/resource?id=123'
        expected_url2 = "https://example.com/resource"
        self.oauth_request.http_url = url2
        self.assertEqual(self.oauth_request.get_normalized_http_url(), expected_url2)
        
        url3 = 'HTTP://Example.com:8080/resource?id=123'
        expected_url3 = "http://example.com:8080/resource"
        self.oauth_request.http_url = url3
        self.assertEqual(self.oauth_request.get_normalized_http_url(), expected_url3)
        
    def test_sign_request(self):
        expected_signature = self.oauth_request.parameters.get(OAuthParameters.SIGNATURE)
        del self.oauth_request.parameters[OAuthParameters.SIGNATURE]
        self.oauth_request.sign_request(self.signature_method, self.consumer, self.token)
        self.assertEqual(self.oauth_request.parameters.get(OAuthParameters.SIGNATURE), expected_signature)

    def test_build_signature(self):
        expected_signature = self.oauth_request.parameters.get(OAuthParameters.SIGNATURE)
        self.assertEqual(self.oauth_request.build_signature(self.signature_method, self.consumer, self.token), 
                         expected_signature)
    
    def test_from_request(self):
        request = oauth.OAuthRequest.from_request(self.http_method, self.http_url, self.oauth_header_with_realm, {}, "message=hey&context_id=")
    ...
    def test_from_consumer_and_token(self):
        request = oauth.OAuthRequest.from_consumer_and_token(self.consumer, self.token, self.http_method, self.http_url)
        self.assertTrue(isinstance(request, oauth.OAuthRequest))
        
    def test_from_token_and_callback(self):
        callback = 'http://example.com'
        request = oauth.OAuthRequest.from_token_and_callback(self.token, callback, self.http_method, self.http_url)
        self.assertTrue(isinstance(request, oauth.OAuthRequest))
    ...
    def test__split_header(self):
        ...
        self.assertEqual(oauth.OAuthRequest._split_header(self.oauth_header_with_realm['Authorization']), self.parameters)
        self.assertEqual(oauth.OAuthRequest._split_header(self.oauth_header_without_realm['Authorization']), self.parameters)
...
class OAuthServerTest(OAuthTest):
    def setUp(self):
        ...
        self.oauth_server = oauth.OAuthServer(self.data_store, {self.signature_method.get_name():self.signature_method})
        
    def _prepare_request(self, request, token = None):
        request.set_parameter(OAuthParameters.SIGNATURE_METHOD, self.signature_method.get_name())
        request.set_parameter(OAuthParameters.NONCE, oauth.generate_nonce())
        request.set_parameter(OAuthParameters.TIMESTAMP, oauth.generate_timestamp())
    ...
    def test_set_data_store(self):
        ...
        self.assertTrue(isinstance(self.oauth_server.data_store, oauth.OAuthDataStore))
    ...
    def test_add_signature_method(self):
        ...
        self.assertTrue(isinstance(self.oauth_server.signature_methods.get(signature_method.get_name()), oauth.OAuthSignatureMethod_PLAINTEXT))
        
    def test_fetch_request_token(self):
        initial_request = oauth.OAuthRequest.from_consumer_and_token(self.consumer,http_method=self.http_method, http_url=self.request_token_url)
        req_token_1 = self._get_token(initial_request)
        authorization_request = oauth.OAuthRequest.from_consumer_and_token(self.consumer, req_token_1, http_method=self.http_method, http_url=self.http_url)
    ...
    def _get_token_for_authorization(self):
        request = oauth.OAuthRequest.from_consumer_and_token(self.consumer,http_method=self.http_method, http_url=self.request_token_url)
        request_token = self._get_token(request)
        authorization_request = oauth.OAuthRequest.from_consumer_and_token(self.consumer, request_token, http_method=self.http_method, http_url=self.http_url)
    ...
    def _get_access_token_request(self, authorized_token):
        access_token_request = oauth.OAuthRequest.from_consumer_and_token(self.consumer, authorized_token, http_method=self.http_method, http_url=self.access_token_url)
    ...
    def test_fetch_access_token(self):
        ...
        self.assertRaises(oauth.OAuthError, self.oauth_server.fetch_access_token, access_token_request)
    ...
    def test_verify_request(self):
        ...
        param1 = 'p1'
        value1 = 'v1'
        api_request = oauth.OAuthRequest.from_consumer_and_token(self.consumer, access_token, http_method=self.http_method, http_url=self.http_url, parameters={param1:value1})
    ...
    def test_get_callback(self):
        request = oauth.OAuthRequest.from_consumer_and_token(self.consumer, None, http_method=self.http_method, http_url=self.http_url)
        self._prepare_request(request)
...
class MockOAuthDataStore(oauth.OAuthDataStore):
    ...
    def fetch_request_token(self, oauth_consumer):
        token = oauth.OAuthToken("5b2cafbf20b11bace53b29e37d8a673dRT", "3f71254637df2002d8819458ae4f6c51RT")
    ...
    def fetch_access_token(self, oauth_consumer, oauth_token):
        ...
        if data:
            ...
            access_token = oauth.OAuthToken("5b2cafbf20b11bace53b29e37d8a673dAT", "3f71254637df2002d8819458ae4f6c51AT")
            self.access_token_db[access_token.key] = (access_token, consumer, authenticated_user)
            return access_token
        else:
            return None
    ...
    def authorize_request_token(self, oauth_token, user):
        ...
        if data and data[2] == None:
            ...
            self.request_token_db[request_token.key] = (request_token, consumer, authenticated_user)
