...
class MultiDomainQuickStartExample(object):
  ...
  def __init__(self, client_id, client_secret, domain):
    ...
    self.multidomain_client = gdata.apps.multidomain.client.MultiDomainProvisioningClient(
  ...
  def _GetChoice(self, for_value):
    choice = int(raw_input(('Enter a choice for %s\n'
                            '1-True 2-False 3-Default/Skip: ') % (for_value)))
    if choice == 1:
      return True
    elif choice == 2:
      return False
  ...
  def _TakeUserData(self, function='create'):
    ...
    while not user_data.first_name:
      user_data.first_name = raw_input('Enter first name for the user: ')
      if function == 'update':
        break
    while not user_data.last_name:
      user_data.last_name = raw_input('Enter last name for the user: ')
      if function == 'update':
        break
  ...
  def _CreateUser(self):
    ...
    response = self.multidomain_client.CreateUser(
...
def main():
  ...
  try:
    ...
    print ('python provisioning_quick_start_example.py --client_id [clientId] '
           '--client_secret [clientSecret] --domain [domain]')
    sys.exit(2)
