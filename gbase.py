...
def _GBaseSnippetFeedFromElementTree(element_tree):
  return atom._XFromElementTree(GBaseSnippetFeed, 'feed', atom.ATOM_NAMESPACE,
      element_tree)
...
class ItemAttribute(atom.Text):
  ...
  #  if element_treedd
  #  self.name = child.tag[child.tag.index('}')+1:]
  #  if attribute == 'type':
...
class GBaseItem(gdata.GDataEntry, ItemAttributeContainer):
  ...
  def _TransferToElementTree(self, element_tree):
    ...
    for name, value in self.item_attributes.items():
      ...
      #new_element.tag = GBASE_TEMPLATE % name
  ...
  def _TakeChildFromElementTree(self, child, element_tree):
    ...
    # Added: finding the the item type
      ...
      #self.item_attributes[name] = child.text
        element_tree.remove(child)
    else:
